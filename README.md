...
- 📸 注ぐ前・注いだ後の写真を1記録に2枚まで添付（WebP自動変換でサイズ最小化）
- ✏️ 既存記録の編集・削除に対応
- 🕰 タイムライン/お気に入りで過去の記録を振り返る参照モード
  ...
  "records": [
    {
      ...
      "rating": 4,
      "memo": "フルーティで飲みやすい",
      "drunk_at": "2026-04-19T14:30:00.000Z",
      "store": "niigata",
      "photos": {
        "before": "data/photos/r_1713345000000_abc12_before.webp",
        "after":  "data/photos/r_1713345000000_abc12_after.webp"
      }
    }
  ]
}
...
- `photos` は任意。添付しなかった場合は省略、または `before` / `after` の片方だけでも可
- 画像は WebP 形式（長辺1200pxにリサイズ、quality 0.75）。1枚あたり50〜150KB程度
- パスは GitHub Pages から直接配信できる相対パス

## セキュリティについて

- PATは端末のlocalStorageに保存されます。**他人が触れる端末では設定しないでください**。
- PATのスコープは**このリポジトリの Contents 権限のみ**に絞ることを強く推奨。万が一漏れても被害を限定できます。
- 記録 (`records.json`) と写真 (`data/photos/`) はPublicリポジトリでは公開されます。見られたくない内容を書かない・写さないか、リポジトリをPrivateに切り替えてください（その場合 GitHub Pages は有料プランが必要）。
...
- 🧹 孤児画像（記録削除後に残った画像ファイル）のクリーンアップスクリプト
